...
- `GET /carrier?pageId=...`：识别正文承载体，返回 `contentKind`、`resourceUrl`、`frameSrc/embedSrc`、`alternativeResources`、`media/scripts` 派生候选等信息
...
- URL 传入 runtime service 时默认保留完整原始地址，包含全部 query 参数与空值参数
- 返回值统一为 JSON
- 出错返回：
  - `ok: false`
  - `error.code`
...
- 已支持单实例端口占用检测
- 已支持正文承载识别：可识别 iframe/embed/object/pdf viewer，并从链接、媒体标签、脚本片段中收集真实资源 URL 候选
- 已支持 PDF extractor：可下载 PDF 并提取文本（优先 fitz，回退 pdfminer）
- 已支持“浏览器上下文下载回退”：当 PDF 直连下载被站点以 412 等方式拦截时，改为借浏览器上下文 fetch 下载再提取
- 已支持多格式基础路由骨架：carrier 识别阶段会收集页面中的 `alternativeResources`，支持 `file/filePath/url/src` 等 viewer 参数解析，供 OFD / Office 等格式优先寻找 PDF 替代资源
...
## 已执行的运行限制
- Runtime Service 默认以后台常驻方式复用，不要求每次任务后停止
...
详细方法论见：`docs/content-extraction.md`

## 后续扩展方向
- OFD 原生 extractor（当前先走替代 PDF/HTML 优先策略）
