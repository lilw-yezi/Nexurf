...
const CONNECT_TIMEOUT_MS = Number(process.env.NEXURF_CONNECT_TIMEOUT_MS || 5000);
...
function classifyResourceKind(resourceUrl = '', text = '') {
  ...
  if (/\.pdf($|[?#&])/i.test(url) || /(^|[^a-z])pdf([^a-z]|$)/.test(label)) return 'pdf';
  if (/\.ofd($|[?#&])/i.test(url) || /(^|[^a-z])ofd([^a-z]|$)/.test(label)) return 'ofd';
  if (/\.(docx?|wps|rtf)($|[?#&])/i.test(url) || /wps|word|docx|doc|rtf|附件|下载/.test(label)) return 'office';
  if (/\.(png|jpe?g|webp|gif|bmp|tiff?)($|[?#&])/i.test(url) || /图片|image|scan|扫描/.test(label)) return 'image';
  if (/viewer|preview|download|attachment|file=|filepath=|url=|src=/.test(url)) return 'candidate';
  return 'resource';
}
...
function uniqueResources(resources) {
  ...
  for (const item of resources) {
    ...
    const key = `${item.kind || ''}::${item.resourceUrl}`;
...
function resolveViewerResource(viewerUrl, baseUrl = '') {
  const normalized = normalizeResourceUrl(viewerUrl, baseUrl);
  if (!normalized) return { resourceUrl: '', viewerUrl: '' };
  try {
    const parsed = new URL(normalized);
    const paramNames = ['file', 'filePath', 'url', 'src', 'href', 'path', 'attach', 'attachment', 'download'];
    for (const name of paramNames) {
      const value = parsed.searchParams.get(name);
      if (value) return { resourceUrl: normalizeResourceUrl(value, normalized), viewerUrl: normalized, param: name };
    }
  } catch {}
  return { resourceUrl: normalized, viewerUrl: normalized };
}
...
async function ensureConnection({ timeoutMs = CONNECT_TIMEOUT_MS } = {}) {
  ...
  connectingPromise = (async () => {
    ...
    if (!browserPort) {
      ...
      if (!discovered) {
        const error = new Error('未发现可用浏览器调试端口，请先在 Chrome 中开启远程调试。');
        lastConnectionError = { message: error.message, at: new Date().toISOString(), stage: 'discover' };
        throw error;
      }
    ...
    await new Promise((resolve, reject) => {
      ...
      const settleReject = (error, stage = 'connect') => {
        if (settled) return;
        settled = true;
        cleanup();
        ...
        browserPort = null;
        browserWsPath = null;
        const msg = error?.message || String(error || '浏览器连接失败');
        lastConnectionError = { message: msg, at: new Date().toISOString(), stage };
      ...
      const timer = setTimeout(() => {
        settleReject(new Error(`浏览器连接超时: ${timeoutMs}ms`), 'timeout');
      }, timeoutMs);
...
function sendCommand(method, params = {}, sessionId = null, timeoutMs = 30000) {
  return new Promise((resolve, reject) => {
    if (!socket || (socket.readyState !== WS.OPEN && socket.readyState !== 1)) {
      const detail = lastConnectionError?.message ? `，最近连接错误: ${lastConnectionError.message}` : '';
      reject(new Error(`浏览器尚未连接${detail}`));
      return;
    }
...
async function detectContentCarrier(sessionId) {
  const response = await sendCommand('Runtime.evaluate', {
    expression: `(() => {
      ...
      const links = Array.from(document.querySelectorAll('a')).map(a => ({
        text: clean(a.textContent || a.title || a.getAttribute('aria-label') || ''),
        href: a.href || '',
        download: a.getAttribute('download') || ''
      })).filter(x => x.href).slice(0, 160);
      const media = Array.from(document.querySelectorAll('iframe,frame,embed,object,source,img,video,audio')).map(el => ({
        tag: el.tagName,
        text: clean(el.textContent || el.title || el.alt || el.getAttribute('aria-label') || ''),
        src: el.src || el.data || el.href || el.currentSrc || '',
        type: el.type || '',
      })).filter(x => x.src).slice(0, 120);
      const scripts = Array.from(document.scripts).map(s => s.src || clean(s.textContent || '').slice(0, 500)).filter(Boolean).slice(0, 40);
      return {
  ...
  const resolved = resolveViewerResource(candidate, raw.pageUrl);
  let finalResourceUrl = resolved.resourceUrl;
  if (/viewer|pdfjs|\b(file|filePath|url|src)=/.test(resolved.viewerUrl || finalResourceUrl)) {
    carrierKind = 'viewer';
  }
  const directKind = classifyResourceKind(finalResourceUrl);
  if (directKind !== 'resource' && directKind !== 'candidate') {
    contentKind = directKind;
    carrierKind = directKind;
  }

  const linkResources = (raw.links || []).map(link => ({
    text: link.text || link.download || '',
    resourceUrl: resolveViewerResource(link.href, raw.pageUrl).resourceUrl,
    source: 'link',
  }));
  const mediaResources = (raw.media || []).map(item => ({
    text: item.text || item.type || item.tag || '',
    resourceUrl: resolveViewerResource(item.src, raw.pageUrl).resourceUrl,
    source: String(item.tag || 'media').toLowerCase(),
  }));
  const scriptResources = (raw.scripts || [])
    .flatMap(text => Array.from(String(text).matchAll(/https?:\/\/[^\s'")<>]+|[A-Za-z0-9_./-]+\.(?:pdf|ofd|docx?|wps|png|jpe?g|webp|gif)(?:\?[^\s'")<>]*)?/gi)).map(m => ({
      text: 'script-resource',
      resourceUrl: normalizeResourceUrl(m[0], raw.pageUrl),
      source: 'script',
    })));
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    if (route === '/health') {
      ...
      json(res, 200, {
        ...
        browser: {
          ...
          port: browserPort,
          connecting: Boolean(connectingPromise),
    ...
    fail(res, 500, 'runtime_service_error', error.message || 'runtime service 执行失败');
  }
});
...
async function main() {
    ...
    try {
      const ok = await new Promise((resolve) => {
        http.get(`http://127.0.0.1:${PORT}/health`, { timeout: 2000 }, (res) => {
          let buf = '';
          res.on('data', c => buf += c);
          res.on('end', () => resolve(buf.includes('nexurf-runtime-service')));
        }).on('error', () => resolve(false));
      });
      if (ok) {
        console.log(`[nexurf] existing runtime service detected on port ${PORT}, exit`);
        process.exit(0);
      }
    } catch {}
  ...
  server.listen(PORT, '127.0.0.1', () => {
    console.log(`[nexurf] runtime service listening on http://127.0.0.1:${PORT}`);
