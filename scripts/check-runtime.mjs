...
const BRIDGE_SCRIPT = path.join(ROOT, 'scripts', 'browser-bridge.mjs');
...
function startBridgeDetached() {
  const logFile = path.join(os.tmpdir(), 'nexurf-bridge.log');
...
async function ensureBridge() {
  ...
  if (current?.service === 'nexurf-bridge') {
    console.log(`bridge: alive (${BRIDGE_PORT})`);
    return true;
  }

  console.log(`bridge: starting (${BRIDGE_PORT})`);
  const logFile = startBridgeDetached();
  await new Promise(resolve => setTimeout(resolve, 1200));

  for (let i = 0; i < 12; i++) {
    const health = await httpGetJson(healthUrl, 2500);
    if (health?.service === 'nexurf-bridge') {
      console.log(`bridge: ready (${BRIDGE_PORT})`);
      return true;
    }
    await new Promise(resolve => setTimeout(resolve, 700));
  }

  console.log(`bridge: failed (${BRIDGE_PORT})`);
  console.log(`bridge-log: ${logFile}`);
  return false;
}
...
async function main() {
  ...
  const profilesDir = path.join(ROOT, 'references', 'site-profiles');
  try {
