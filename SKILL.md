...
```bash
node "${CLAUDE_SKILL_DIR}/scripts/check-runtime.mjs"
```
...
需要多格式提取细节时，读取：`references/content-extraction.md`
...
```bash
node "${CLAUDE_SKILL_DIR}/scripts/match-profile.mjs" "用户请求或目标站点"
```

若命中画像，优先复用已有经验。

站点画像位于：
- `references/site-profiles/_template.md`
- `references/site-profiles/<domain>.md`
...
## 何时读取额外参考
- 需要 bridge 动作与接口边界：读取 `references/browser-api.md`
- 需要多格式提取方法：读取 `references/content-extraction.md`
- 已确定目标站点：读取对应的 `references/site-profiles/<domain>.md`
